Attribute VB_Name = "mdlRTFtoHTML"
...
Function OH_rtf2html3(strRTF As String, Optional strOptions As String) As String
    'Options:
    ...
    ClearFont
    '======
    strT = strHTML '210217 ergänzt wegen HVL mails
    If left(strT, 5) = "<div>" And right(strT, 1) = ">" Then
        strT = Mid(strT, 6, Len(strT) - 11)
        If InStr(strT, "<font size") = 0 Then
            strSize = ";font-size:" & glstrFontSizeEmail & "pt'>"
        Else
            strSize = "'>"
        End If
        strT = Replace(strT, "</div><div>", "<br>")
        strT = "<p style='font-family:" & glstrFontEmail & strSize & strT & "</p>"
    End If
...
Function OH_GetBoiler(ByVal sFile As String) As String
'Dick Kusleika
'http://www.rondebruin.nl/mail/folder3/signature.htm
'FUNKTIONIERT NICHT wenn Bilder drinne sind!! also weg damit
    Dim fso As Object
    Dim ts As Object
    If Len(Dir(sFile)) > 0 Then
        Set fso = CreateObject("Scripting.FileSystemObject")
        Set ts = fso.GetFile(sFile).OpenAsTextStream(1, -2)
        OH_GetBoiler = ts.ReadAll
        ts.Close
    Else
        OH_GetBoiler = "Nicht gefunden: " & sFile
    End If
End Function
